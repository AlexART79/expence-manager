...
export function createCategoriesRouter(db: Db): Router {
  ...
  router.get('/', (req, res) => {
    if (!req.user) {
      return res
        .status(401)
        .json({ error: { code: 'UNAUTHORIZED', message: 'Not authenticated', details: {} } });
    }
  ...
  router.post('/', validateRequest({ body: CategoryBodySchema }), (req, res, next) => {
    ...
    try {
      ...
      if (err instanceof DuplicateCategoryNameError) {
        return res
          .status(409)
          .json({ error: { code: 'CONFLICT', message: err.message, details: {} } });
      }
      next(err);
    }
  });
    ...
    (req, res, next) => {
      ...
      try {
        ...
        if (err instanceof CategoryNotFoundError) {
          return res
            .status(404)
            .json({ error: { code: 'NOT_FOUND', message: err.message, details: {} } });
        }
        if (err instanceof DuplicateCategoryNameError) {
          return res
            .status(409)
