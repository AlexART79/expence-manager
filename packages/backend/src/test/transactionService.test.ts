...
describe('transactionService', () => {
  ...
  it("createTransaction throws CategoryNotOwnedError for another user's category", () => {
    ...
    try {
      expect(() => createTransaction(db, 999, { ...baseInput, categoryId: cat.id })).toThrow(
        CategoryNotOwnedError,
      );
    } finally {
      sqlite.close();
  ...
  it("listTransactions returns only the current user's transactions", () => {
    ...
    try {
      ...
      const [cat2] = db
        .insert(categories)
        .values({ userId: user2!.id, name: 'Food' })
        .returning()
        .all();
  ...
  it('listTransactions filters by categoryId', () => {
    ...
    try {
      const [cat2] = db
        .insert(categories)
        .values({ userId: user.id, name: 'Transport' })
        .returning()
        .all();
  ...
  it('updateTransaction updates the transaction', () => {
    ...
    try {
      ...
      const updated = updateTransaction(db, user.id, created.id, {
        ...baseInput,
        categoryId: cat.id,
        title: 'Updated',
        amount: 99,
  ...
  it('updateTransaction throws TransactionNotFoundError for wrong userId', () => {
    ...
    try {
      const created = createTransaction(db, user.id, { ...baseInput, categoryId: cat.id });
      expect(() =>
        updateTransaction(db, 999, created.id, { ...baseInput, categoryId: cat.id }),
  ...
  it("updateTransaction throws CategoryNotOwnedError for another user's category", () => {
    ...
    try {
      ...
      const created = createTransaction(db, user.id, { ...baseInput, categoryId: cat.id });
      expect(() =>
        updateTransaction(db, user.id, created.id, { ...baseInput, categoryId: cat2!.id }),
